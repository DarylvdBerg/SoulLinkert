...
export const AddPairForm = ({locations, pokemonList}: {locations: string[], pokemonList: string[]}): JSX.Element => {
    ...
    async function addPairOnSubmit() {
        if(location === '' || location == undefined || 
            pokemonOne === '' || pokemonOne === undefined ||
            pokemonTwo === '' || pokemonTwo === undefined) {
            setHideError(false)
            return;
        }
    ...
    return (
        <Flex direction="column" gap={4}>
            ...
            <Text color='red' alignSelf='center' hidden={hideError}>Unable to add pair, please check if all data is filled in.</Text>
            <Selector collection={locations} selectLabel="Location caught" selectValueText="Pokemon caught on.." onSelectCallCapture={setLocation} />
            <Selector collection={pokemonList} selectLabel="Pokemon 1" selectValueText="Select which pokemon was caught" onSelectCallCapture={setPokemonOne} />
            <Selector collection={pokemonList} selectLabel="Pokemon 2" selectValueText="Select which pokemon was caught" onSelectCallCapture={setPokemonTwo} />
            <Button w='80%' alignSelf='center' onClick={addPairOnSubmit}>Add pair</Button>
        </Flex>
    )
}
